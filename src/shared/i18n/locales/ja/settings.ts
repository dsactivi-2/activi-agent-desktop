export default {
  ...
  poolHint:
    "同じプロバイダの API キーを複数追加して自動ローテーション・負荷分散。Activi Agent が順に使い回します。",
  add: "追加",
  remove: "削除",
  keyLabel: "キー",
  empty: "（空）",
  dataSection: "データ",
  dataHint:
    "Activi Agent の設定、セッション、スキル、メモリのエクスポート・インポートを行います。",
  backingUp: "バックアップ中...",
  ...
  migrationDesc:
    "<code>{{path}}</code> に OpenClaw が見つかりました。設定・API キー・セッション・スキルを Activi Agent に移行できます。",
  migrationDismiss: "再表示しない",
  migrating: "移行中...",
  migrateToHermes: "Activi Agent に移行",
  skip: "スキップ",
  ...
  modeLocal: "ローカル",
  modeRemote: "リモート",
  modeLocalHint: "このデバイスにインストールされた Activi Agent を使用",
  modeRemoteHint:
    "ネットワークまたはクラウド上の Activi Agent API サーバに接続",
  remoteUrl: "リモート URL",
  remoteUrlHint:
    "Activi Agent API サーバの URL（/health と /v1/chat/completions を公開している必要あり）",
  remoteApiKey: "API キー",
  remoteApiKeyHint:
  ...
  serverConfigHint:
    "リモート Activi Agent サーバに接続中です。モデル選択、プロバイダ API キー、認証情報はサーバ側の <code>~/.hermes/.env</code> と <code>config.yaml</code> で管理されます。ホスト側で編集（例：<code>docker exec -it hermes vi /opt/data/.env</code>）してコンテナを再起動してください。",
  connectionMode: "モード",
